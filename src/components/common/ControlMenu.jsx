...
//   const handleARView = () => {
//   if (!selectedItem) {
//     alert(`No furniture item selected for AR view.`);
//     return;
//   }

//   // If uploaded item has direct URL
//   if (selectedItem.arFileUrl) {
//     const baseUrl = window.location.origin;
//     const arViewerUrl = `${baseUrl}/ar-viewer?model=${encodeURIComponent(selectedItem.arFileUrl)}&name=${encodeURIComponent(selectedItem.name)}`;
//     window.open(arViewerUrl, "_blank"); // 👉 open directly in new tab
//     return;
//   }

//   // Fallback: fetch AR URL from API
//   setIsGeneratingQR(true);
...
//     },
//     onError: (error) => {
//       console.error("Error generating AR view link:", error);
//       setIsGeneratingQR(false);
//     }
//   });
// };
...
//     const generateQRCodeWithUrl = (modelUrl) => {
//       const baseUrl = window.location.origin;
//       const arViewerUrl = `${baseUrl}/ar-viewer?model=${encodeURIComponent(modelUrl)}&name=${encodeURIComponent(selectedItem.name)}`;
...
//     if (selectedItem.arFileUrl) {
//       generateQRCodeWithUrl(selectedItem.arFileUrl);
//       return;
//     }
...
//       onSuccess: (data) => {
//         if (data?.arFileUrl) {
//           generateQRCodeWithUrl(data.arFileUrl);
//         } else {
//           alert("AR file URL not available.");
//           setIsGeneratingQR(false);
//         }
//       },
//       onError: (error) => {
//         console.error("Error generating AR view link:", error);
//         alert("Error generating AR link.");
//         setIsGeneratingQR(false);
//       }
//     });
//   };
...
//     <>
//       {/* Toggle Floating Button */}
//       <div className="fixed top-1/2 transform -translate-y-1/2 z-50" style={{ left: "25%", top: "30%" }}>
//         <button
//           onClick={() => setIsMenuExpanded(!isMenuExpanded)}
//           className={`bg-gradient-to-br from-blue-500 to-purple-600 text-white p-3 rounded-full shadow-2xl hover:shadow-3xl transition-all duration-300 transform hover:scale-110 ${isMenuExpanded ? 'rotate-90' : 'rotate-0'}`}
//           style={{ boxShadow: '0 10px 25px rgba(59, 130, 246, 0.4), 0 0 20px rgba(147, 51, 234, 0.3)' }}
//         >
//           <Menu size={18} />
//         </button>

//         {/* Expanded Floating Menu */}
//         <div className={`absolute left-0 top-16 bg-white/50 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 p-4 min-w-[280px] transition-all duration-500 transform origin-top-left ${isMenuExpanded ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-75 -translate-y-4 pointer-events-none'}`}>
//           {/* Header */}
//           <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-200/50">
//             <h3 className="text-lg font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
//               Controls
//             </h3>
//             <button onClick={onClose} className="text-gray-400 hover:text-red-500 transition-colors duration-200 p-1">
//               <X size={16} />
//             </button>
//           </div>
...
//             <MobileIconBtn icon={<Ruler />} label="Dimensions" onClick={() => handleShowDimensions(selectedModelId)} color="teal" />
//             <button
//               className={`flex flex-col items-center justify-center rounded-xl p-3 transition-all duration-300 transform hover:scale-105 ${isGeneratingQR || !selectedModelId ? 'opacity-50 cursor-not-allowed bg-gray-100' : 'bg-pink-50 text-pink-600 hover:bg-pink-100 hover:shadow-lg'}`}
//               onClick={handleARView}
//               disabled={isGeneratingQR || !selectedModelId}
//               title="AR View"
//             >
...
// function QRPopup({ qrCodeData, closeQRPopup, handleCopyLink, handleOpenInBrowser }) {
//   return (
//     <div className="fixed inset-0 flex justify-center items-center z-50" onClick={closeQRPopup}>
//       <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4 relative flex flex-col items-center animate-pulse-scale" onClick={(e) => e.stopPropagation()}>
//         <button onClick={closeQRPopup} className="absolute top-4 right-4 text-gray-500 hover:text-red-600 transition-colors duration-300 focus:outline-none">
//           <FaTimes size={24} />
//         </button>
//         <h2 className="text-2xl font-bold text-gray-900 mb-2">AR Viewer</h2>
//         <div className="text-lg font-semibold text-gray-700 mb-6 text-center">{qrCodeData.modelName || "3D Model"}</div>
//         <div className="relative mb-4">
//           <div className="absolute inset-0 bg-gradient-to-r from-purple-400 to-pink-400 blur-lg opacity-30 rounded-lg"></div>
//           <div className="relative border-3 border-transparent bg-gradient-to-r from-purple-500 to-pink-500 rounded-lg p-1">
//             <div className="bg-white rounded-md p-2">
...
//         </div>
//         <div className="flex flex-col gap-3 mb-10 px-2 w-full">
//           <button onClick={handleOpenInBrowser} className="bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-full px-6 py-3 font-semibold hover:from-purple-700 hover:to-pink-700 transition-all duration-300 focus:outline-none shadow-lg transform hover:scale-105">
//             🚀 Open AR Viewer Now
//           </button>
//           <div className="flex gap-3">
//             <button onClick={handleCopyLink} className="flex-1 bg-blue-600 text-white rounded-full px-4 py-2 font-semibold hover:bg-blue-700 transition-colors duration-300 focus:outline-none shadow-md">
//               📋 Copy Link
//             </button>
//             <button onClick={closeQRPopup} className="flex-1 bg-gray-300 text-gray-800 rounded-full px-4 py-2 font-semibold hover:bg-gray-400 transition-colors duration-300 focus:outline-none shadow-md">
//               Close
//             </button>
//           </div>
//         </div>
...
export default function ControlMenu({
  ...
  const handleARView = () => {
    ...
    const arUrl = selectedItem?.arFileUrl;
    if (!arUrl) {
      alert("AR file URL not found for this item.");
      return;
    }
    ...
    const baseUrl = window.location.origin;
    const arViewerUrl = `${baseUrl}/ar-viewer?model=${encodeURIComponent(arUrl)}&name=${encodeURIComponent(selectedItem.name)}`;
    const qrCodeUrl = `https://api.qrserver.com/v1/create-qr-code/?data=${encodeURIComponent(arViewerUrl)}&size=250x250`;
  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center z-50" onClick={closeQRPopup}>
      ...
      >
        ...
        <h2 className="text-2xl font-bold mb-2">AR Viewer</h2>
        <div className="text-lg font-semibold text-gray-700 mb-6 text-center">
          {qrCodeData.modelName}
        </div>
        ...
        <div className="text-center text-sm mb-6 px-2 bg-gradient-to-r from-blue-50 to-purple-50 p-4 rounded-lg border border-purple-200">
          <p className="font-semibold mb-2">📱 Scan with phone camera</p>
          <p>To view this furniture in AR</p>
        </div>

        <div className="flex flex-col gap-3 px-2 w-full">
          <button
            onClick={handleOpenInBrowser}
            className="bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-full px-6 py-3 font-semibold hover:from-purple-700 hover:to-pink-700"
          >
            🚀 Open AR Viewer
          </button>
          <div className="flex gap-3">
            <button
              onClick={handleCopyLink}
              className="flex-1 bg-blue-600 text-white rounded-full px-4 py-2 font-semibold hover:bg-blue-700"
            >
              📋 Copy Link
            </button>
            <button
              onClick={closeQRPopup}
              className="flex-1 bg-gray-300 text-gray-800 rounded-full px-4 py-2 font-semibold hover:bg-gray-400"
            >
              Close
            </button>
          </div>
