...
//           this.offset.set(0, 0, 0);
//         }
//         // Calculate initial bottom offset via bounding box.
//         const mesh = this.el.getObject3D("mesh");
//         if (mesh) {
//           const bbox = new AFRAME.THREE.Box3().setFromObject(this.el.object3D);
//           this.initialBottomOffset =
//             this.el.object3D.position.y - bbox.min.y;
//         } else {
//           this.initialBottomOffset = 0;
//         }
//         window.addEventListener("mousemove", this.onMouseMove);
...
//         }
//         const targetPos = intersectionPoint.clone().add(this.offset);
//         // Optional clamping based on room boundaries.
//         if (window.roomBounds) {
//           const box = new AFRAME.THREE.Box3().setFromObject(this.el.object3D);
//           const halfWidth = (box.max.x - box.min.x) / 2;
//           const halfDepth = (box.max.z - box.min.z) / 2;
...
//   // and clamp the position so the menu never leaves the viewport.
//   // --------------------------------------------------------------------
//   useEffect(() => {
//     let animationFrameId;
//     const updateMenuPosition = () => {
...
//   // (Optional) Additional effects like enforceAboveGround can remain as before.
//   const enforceAboveGround = (modelEl) => {
//     if (!modelEl) return;
//     const mesh = modelEl.getObject3D("mesh");
//     if (!mesh) return;
...
//             onClick={() => handleAddItem(item.src)}
//           >
//             <img src={item.thumbnail} alt={item.name} className="w-full h-20 object-contain rounded-lg" />
//             <p className="text-center text-white mt-2">{item.name}</p>
//           </div>
//         ))}
//       </div>
