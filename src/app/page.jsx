...
//     AFRAME.registerComponent("bounding-box-helper", {
//       schema: {
//         color: { type: "color", default: "#77dd77" },
//       },
//       init: function () {
...
//   const [cursorPos, setCursorPos] = useState("0 1 0");
//   const [menuPosition, setMenuPosition] = useState(null);
//   // Dimension popups for width, height, and depth.
//   const [dimensionPopups, setDimensionPopups] = useState({
//     width: null,
//     height: null,
//     depth: null,
//   });
...
//   const menuRef = useRef(null); // ref for the control menu
...
//     { src: "/bed-13.glb", thumbnail: "/bed-13.png", name: "Bed" },
//     {
//       src: "/claudia_3_seater_sofa_peacock_blue_velvet.glb",
//       thumbnail: "/Blue Seater.png",
//       name: "Seater",
//     },
//     { src: "/designer_chair_01c.glb", thumbnail: "/Green Chair.png", name: "Chair" },
//     {
//       src: "/helena_large_corner_sofa_plush_turmeric_velvet.glb",
//       thumbnail: "/Corner Sofa.png",
//       name: "Corner Sofa",
//     },
//     { src: "/sofa_chair.glb", thumbnail: "/sofa_chair.png", name: "sofaChair" },
//     { src: "/akia_hand_woven_rug_160_x_230cm.glb", thumbnail: "/Hand Rug.png", name: "Cupboard" },
...
//     };
//     setModels([...models, model]);
//     // Clear selection so that no bounding box is visible.
//     setSelectedModelId(null);
//     setMenuPosition(null);
//     setShowDimensionPopup(false);
//     setModelId(modelId + 1);
...
//       setCursorPos(newPos);
//     }
//     // When clicking the floor, clear selection.
//     setSelectedModelId(null);
//     setMenuPosition(null);
//     setShowDimensionPopup(false);
//   };
...
//   // Helper: Clear all bounding boxes.
...
//   // When clicking a model, hide bounding boxes on previously selected items.
//   const handleModelClick = (evt, model) => {
//     evt.stopPropagation();
//     clearAllBoundingBoxes();
//     setSelectedModelId(model.id);
//     // Optionally hide dimensions when switching.
//     setShowDimensionPopup(false);
//   };

//   // Update control menu position based on the selected model's bounding box center.
//   useEffect(() => {
//     let animationFrameId;
//     const updateMenuPosition = () => {
//       if (selectedModelId !== null) {
...
//           // Clamp with a base margin.
//           x = Math.max(margin, Math.min(x, window.innerWidth - margin));
//           y = Math.max(margin, Math.min(y, window.innerHeight - margin));

//           // Adjust based on menu size if ref is available.
//           if (menuRef.current) {
...
//   // Returns dimensions from the object's bounding box.
//   const getModelDimensions = (modelId) => {
//     const modelEl = document.getElementById(modelId);
//     if (modelEl) {
//       const mesh = modelEl.getObject3D("mesh");
...
//   // When Dimensions button is clicked, activate dimension popups
//   // and add the bounding-box-helper component (to show the green bounding box).
//   const handleShowDimensions = (id) => {
//     setShowDimensionPopup(true);
//     clearAllBoundingBoxes();
//     const modelEl = document.getElementById(id);
...
//   // Continuously update the dimension popups' positions and text.
//   useEffect(() => {
//     if (!showDimensionPopup || !selectedModelId) {
//       setDimensionPopups({ width: null, height: null, depth: null });
//       return;
//     }
//     let animationFrameId;
//     const updateDimensionPopups = () => {
//       const dims = getModelDimensions(selectedModelId);
//       const cameraEl = document.querySelector("a-camera");
//       const cameraObj = cameraEl && cameraEl.getObject3D("camera");
//       if (dims && cameraObj) {
//         // Define offsets in world units.
//         const offsetY = 0.3;
//         const offsetX = 0.3;

//         // For width: center of the top edge.
//         const widthEdgeCenter = new THREE.Vector3(
//           (dims.min.x + dims.max.x) / 2,
//           dims.max.y,
//           dims.max.z
//         );
//         const widthLabelWorld = widthEdgeCenter.clone().add(new THREE.Vector3(0, offsetY, 0));

//         // For height: center of the left vertical edge.
//         const heightEdgeCenter = new THREE.Vector3(
//           dims.min.x,
//           (dims.min.y + dims.max.y) / 2,
//           dims.min.z
//         );
//         const heightLabelWorld = heightEdgeCenter.clone().add(new THREE.Vector3(-offsetX, 0, 0));

//         // For depth: center of the front edge.
//         const depthEdgeCenter = new THREE.Vector3(
//           dims.min.x,
//           dims.min.y,
//           (dims.min.z + dims.max.z) / 2
//         );
//         const depthLabelWorld = depthEdgeCenter.clone().add(new THREE.Vector3(0, offsetY, -offsetX));

//         const widthScreen = projectToScreen(widthLabelWorld, cameraObj);
//         const heightScreen = projectToScreen(heightLabelWorld, cameraObj);
//         const depthScreen = projectToScreen(depthLabelWorld, cameraObj);

//         setDimensionPopups({
//           width: {
//             x: widthScreen.x,
//             y: widthScreen.y,
//             text: `Width: ${dims.width.toFixed(2)}`,
//           },
//           height: {
//             x: heightScreen.x,
//             y: heightScreen.y,
//             text: `Height: ${dims.height.toFixed(2)}`,
//           },
//           depth: {
//             x: depthScreen.x,
//             y: depthScreen.y,
//             text: `Length: ${dims.depth.toFixed(2)}`,
//           },
//         });
//       }
//       animationFrameId = requestAnimationFrame(updateDimensionPopups);
//     };
//     updateDimensionPopups();
//     return () => cancelAnimationFrame(animationFrameId);
//   }, [showDimensionPopup, selectedModelId, models, menuPosition]);
...
//             <div
//               ref={menuRef}
//               className="absolute bg-white bg-opacity-85 rounded shadow grid grid-cols-6 gap-1"
//               style={{
//                 width: "240px",
//                 height: "100px",
//                 left: menuPosition.left,
...
//               </button>
//               <button
//                 className="flex flex-col items-center justify-center hover:text-blue-600 focus:outline-none ml-7"
//                 onClick={() => handleShowDimensions(selectedModelId)}
//               >
//                 <MdStraighten className="text-xs" />
//                 <span className="text-xs">Dimensions</span>
//               </button>
//               {/* Add an empty placeholder cell to complete the 12-grid layout */}
//               <div></div>
//             </div>
//           )}

//           {/* Dimension Popups for width, height, and depth */}
//           {showDimensionPopup && dimensionPopups.width && (
//             <div
//               style={{
//                 position: "absolute",
//                 left: dimensionPopups.width.x,
//                 top: dimensionPopups.width.y,
...
//               }}
//             >
//               {dimensionPopups.width.text}
//             </div>
//           )}
//           {showDimensionPopup && dimensionPopups.height && (
//             <div
//               style={{
//                 position: "absolute",
//                 left: dimensionPopups.height.x,
//                 top: dimensionPopups.height.y,
...
//               }}
//             >
//               {dimensionPopups.height.text}
//             </div>
//           )}
//           {showDimensionPopup && dimensionPopups.depth && (
//             <div
//               style={{
//                 position: "absolute",
//                 left: dimensionPopups.depth.x,
//                 top: dimensionPopups.depth.y,
//                 transform: "translate(-330%, -400%)",
//                 background: "#f5f5f5",
//                 color: "#696969",
//                 padding: "2px 4px",
//                 borderRadius: "3px",
...
//   const [cursorPos, setCursorPos] = useState("0 1 0");
//   const [menuPosition, setMenuPosition] = useState(null);
//   // State for dimension popup container
...
//           // initial screen coordinates from the model's center projection
//           let x = (center.x * 0.5 + 0.5) * window.innerWidth;
//           let y = (-center.y * 0.5 + 0.5) * window.innerHeight;
//           const margin = 20;
...
//   // Continuously update the dimension popup container so it follows the model.
//   useEffect(() => {
//     let animId;
...
//           // Position the dimensions container slightly above the model.
//           setDimContainerPos({ left: proj.x, top: proj.y - 40 });

//           // Also update the dimensions text.
//           const mesh = modelEl.getObject3D("mesh");
//           if (mesh) {
...
// import * as THREE from "three";
// import { GLTFLoader } from "three/examples/jsm/loaders/GLTFLoader";
// // import { QRCodeCanvas } from "qrcode.react"; // QR Code generator library.
...
//     AFRAME.registerComponent("bounding-box-helper", {
//       schema: {
//         color: { type: "color", default: "#77dd77" }, // Updated to match first code.
//       },
//       init: function () {
...
//   const [cursorPos, setCursorPos] = useState("0 1 0");
//   const [menuPosition, setMenuPosition] = useState(null);
//   // NEW: State for dimensions popup container and text (same as first code)
//   const [dimContainerPos, setDimContainerPos] = useState(null);
//   const [dimensionsText, setDimensionsText] = useState({
//     width: "",
//     height: "",
...
//   // Ref for custom furniture file input.
...
//   // ------------------------------
//   // Update control menu position based on model's bounding box center.
//   // (Same logic and design as in your first code)
//   useEffect(() => {
//     let animationFrameId;
//     const updateMenuPosition = () => {
//       if (selectedModelId !== null) {
...
//           x = Math.max(margin, Math.min(x, window.innerWidth - margin));
//           y = Math.max(margin, Math.min(y, window.innerHeight - margin));
//           // Adjust based on the menu size if ref available.
//           if (menuRef.current) {
//             const { offsetWidth: menuWidth, offsetHeight: menuHeight } = menuRef.current;
//             if (x + menuWidth / 2 > window.innerWidth - margin) {
//               x = window.innerWidth - margin - menuWidth / 2;
...
//   // ------------------------------
//   // Update dimension popup container (position and dimension text) to follow the model.
//   useEffect(() => {
//     let animId;
//     const updateDimPopupPosition = () => {
//       if (showDimensionPopup && selectedModelId) {
...
//           // Position the dimensions container slightly above the model.
//           setDimContainerPos({ left: proj.x, top: proj.y - 40 });
//           // Also update dimensions text using the mesh bounding box.
//           const mesh = modelEl.getObject3D("mesh");
//           if (mesh) {
//             const meshBox = new THREE.Box3().setFromObject(mesh);
//             const min = meshBox.min;
...
//   // When Dimensions button is clicked, activate the popup and add the bounding-box-helper.
//   const handleShowDimensions = (id) => {
//     setShowDimensionPopup(true);
//     clearAllBoundingBoxes();
//     const modelEl = document.getElementById(id);
...
//           {/* Updated Control Menu for selected model 
//               (Using same design as your first code: grid layout, fixed width/height, and adjusted transform.) */}
//           {selectedModelId !== null && menuPosition && (
//             <div
//               ref={menuRef}
//               className="absolute bg-white bg-opacity-85 rounded shadow grid grid-cols-6 gap-2 p-3"
//               style={{
//                 width: "280px",
//                 height: "100px",
//                 left: menuPosition.left,
...
//                 <span className="text-xs">Dimensions</span>
//               </button>
//               {/* Optional: Add another button for AR view or QR Code if needed */}
...
//           {/* Updated Dimension Popup Container – styled identically as in your first code */}
//           {showDimensionPopup && dimContainerPos && (
//             <div
//               style={{
//                 position: "absolute",
...
//         </div>
//       </div>
//       {/* Optionally, your QR Code overlay for AR view can be placed here */}
//       {/* {qrVisible && (
//         <div 
//           style={{
//             position: "absolute",
...
//   const [cursorPos, setCursorPos] = useState("0 1 0");
//   const [menuPosition, setMenuPosition] = useState(null);
//   // NEW: State for dimensions popup container and text.
//   const [dimContainerPos, setDimContainerPos] = useState(null);
//   const [dimensionsText, setDimensionsText] = useState({
//     width: "",
//     height: "",
...
//               </button>
//               <button
//                 className="flex flex-col items-center justify-center hover:text-blue-600 focus:outline-none ml-2 "
//                 onClick={() => handleShowDimensions(selectedModelId)}
//               >
//                 <MdStraighten className="text-xs" />
//                 <span className="text-xs">Dimensions</span>
//               </button>
//               <button className="flex flex-col items-center justify-center hover:text-blue-600 focus:outline-none ml-2">
//                 <span className="text-xs">AR View</span>
//               </button>
//             </div>
//           )}
...
//     AFRAME.registerComponent("bounding-box-helper", {
//       schema: {
//         // Default color is set here but will be overridden if provided as an attribute.
//         color: { type: "color", default: "#77dd77" },
//       },
//       init: function () {
//         // Wait until the model is loaded.
//         this.el.addEventListener("model-loaded", () => {
//           // Create a box helper using the given color.
//           this.helper = new THREE.BoxHelper(this.el.object3D, this.data.color);
//           this.el.sceneEl.object3D.add(this.helper);
//         });
//       },
...
//   // When the Dimensions button is clicked, activate the popup and add the green bounding-box-helper.
//   const handleShowDimensions = (id) => {
//     setShowDimensionPopup(true);
//     // Remove existing bounding boxes from all models.
//     clearAllBoundingBoxes();
//     const modelEl = document.getElementById(id);
//     if (modelEl && !modelEl.hasAttribute("bounding-box-helper")) {
//       // Set the bounding box helper with green color.
//       modelEl.setAttribute("bounding-box-helper", "color: green");
//     }
//   };
...
//           ).sub(evt.detail.intersection.point);
//           this.offset.y = 0;
//         } else {
//           this.offset.set(0, 0, 0);
//         }
...
//     AFRAME.registerComponent("bounding-box-helper", {
//       schema: {
//         // Default color defined here.
//         color: { type: "color", default: "#77dd77" },
//       },
//       init: function () {
//         // Wait until the model is loaded.
//         this.el.addEventListener("model-loaded", () => {
//           // Create a BoxHelper using the model's object3D and the given color.
//           this.helper = new THREE.BoxHelper(this.el.object3D, this.data.color);
//           this.el.sceneEl.object3D.add(this.helper);
//         });
//       },
...
//     const newModels = models.map((model) => {
//       if (model.id === id) {
//         const currentRotation = AFRAME.utils.coordinates.parse(
...
//     const modelEl = document.getElementById(id);
//     if (modelEl && !modelEl.hasAttribute("bounding-box-helper")) {
//       // Set the bounding box helper without overriding the default color.
//       modelEl.setAttribute("bounding-box-helper", "");
//     }
//   };
