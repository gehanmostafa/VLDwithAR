...
import StartButton from '../components/ui/StartButton'; // Ensure this path is correct
    ...
    <main className="bg-[#0d4c3e] font-sans text-white">
      ...
      <section className="py-14 px-4 sm:px-6 md:px-10 bg-[#0d4c3e] text-white">
        <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          ...
          {[
            { tag: '#AR', title: 'Augmented Reality', text: 'Our AR technology allows customers to preview their furniture in a virtual room.' },
            { tag: '#AI', title: 'Artificial Intelligence', text: 'Our AI algorithm recommends designs based on your style and room layout.' },
            { tag: '#Free', title: 'Design Consultations', text: 'Get expert advice and design consultations free of charge.' },
          ].map(({ tag, title, text }, i) => (
            <div key={i} className="bg-[#f3f8f7] text-[#0d4c3e] px-6 py-8 rounded-xl transition-transform duration-300 transform hover:scale-[1.03] hover:shadow-xl h-full">
              <p className="font-semibold text-sm mb-2">{tag}</p>
              <h3 className="font-bold text-lg mb-1">{title}</h3>
              <p className="text-sm">{text}</p>
            </div>
        ...
        <div className="mt-10 max-w-6xl mx-auto flex justify-center md:justify-end">
          <button
            ...
            className="group relative flex items-center gap-3 bg-white text-[#0d4c3e] px-6 sm:px-8 py-2 sm:py-3 rounded-full font-semibold transition-all duration-300 hover:pl-10 hover:shadow-md"
          >
            <span className="transition-all duration-300">Sign up</span>
      ...
      {/* ---------- Testimonial Section ---------- */}
      <section className="bg-[#0d4c3e] py-16 px-4 sm:px-6">
        <div className="bg-[#f3f8f7] max-w-6xl mx-auto rounded-xl px-4 sm:px-6 py-10 sm:py-12">
          <h2 className="text-xl sm:text-2xl font-bold text-[#0d4c3e] text-center mb-10">Customer Testimonial</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {[
              { name: 'Olivia Wilson', role: 'Founder of Roodle', comment: 'Awesome tool!', feedback: 'Lorem ipsum praesent ac massa quisque sodales quis erat euismod.' },
              { name: 'Matt Zhang', role: 'UI/UX Lead', comment: 'Super cool!', feedback: "I use this every day — it's insanely useful and smooth." },
              { name: 'Hannah Morales', role: 'COO at BeeView', comment: 'Loved it!', feedback: 'A must-have tool for creatives and developers. Just wow.' },
            ].map(({ name, role, comment, feedback }, i) => (
              <div key={i} className="bg-[#0d4c3e] text-white rounded-xl p-6 min-h-[300px] flex flex-col justify-between shadow-lg hover:scale-105 transition-transform duration-300">
                <div>
                  <div className="text-3xl text-white mb-2">“</div>
                  <p className="text-lg font-bold">{comment}</p>
                  <p className="text-sm mt-2 mb-4 leading-relaxed text-white/80">{feedback}</p>
      ...
      {/* ---------- Contact Section ---------- */}
      <section className="bg-[#0d4c3e] py-16 px-4">
        <div className="bg-[#f3f8f7] max-w-6xl mx-auto rounded-xl px-4 sm:px-6 py-12">
          <h2 className="text-[#0d4c3e] text-lg sm:text-xl font-bold mb-10 flex items-center gap-2">
            <span className="w-4 h-4 bg-[#0d4c3e] rounded-sm"></span>
            Contact Us
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
            <div className="relative w-fit mx-auto group">
              <img src="/1.jpg" alt="Back" className="absolute top-0 left-0 w-[280px] sm:w-[360px] h-[240px] object-cover rounded-xl shadow-md transform rotate-[-10deg] -translate-x-14 translate-y-12 opacity-70 scale-90 group-hover:translate-x-2 group-hover:translate-y-4 group-hover:rotate-[-3deg] group-hover:opacity-100 group-hover:scale-100" />
              <img src="/2.jpg" alt="Middle" className="absolute top-0 left-0 w-[280px] sm:w-[360px] h-[240px] object-cover rounded-xl shadow-lg transform rotate-[-5deg] -translate-x-6 translate-y-6 opacity-80 scale-95 group-hover:z-30 group-hover:translate-x-0 group-hover:-translate-y-4 group-hover:rotate-[0deg] group-hover:scale-[1.05] group-hover:opacity-100" />
              <img src="/3.jpg" alt="Front" className="relative w-[280px] sm:w-[360px] h-[240px] object-cover rounded-xl shadow-2xl transform rotate-[2deg] z-20 group-hover:z-10 group-hover:translate-x-2 group-hover:-translate-y-2 group-hover:brightness-90" />
            </div>
            <div className="relative w-full max-w-md mx-auto bg-[#0d4c3e] rounded-2xl p-6 sm:p-8 text-white shadow-2xl space-y-5 border border-[#ffffff22] flex flex-col items-center justify-center h-[340px]">
              <div className="text-center space-y-4">
                <h3 className="text-lg font-semibold">Let's get in touch</h3>
